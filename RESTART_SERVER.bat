...
if %errorlevel% equ 0 (
    ...
    echo [INFO] No node.exe process found
)
...
node -e "const fetch = require('node-fetch'); fetch('https://letsbunk-uw7g.onrender.com/api/health').then(r => r.json()).then(d => console.log('[OK] Server is responding:', d.status)).catch(e => console.log('[ERROR] Server not responding:', e.message))"
